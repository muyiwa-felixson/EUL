...
import { Line, LineChart, ResponsiveContainer, YAxis, XAxis, CartesianGrid, Tooltip, Label, Legend } from 'recharts';
        ...
        <ResponsiveContainer width="100%" aspect={props.aspect  && 3} height={props.aspect ? false : props.height}>
            ...
            props.tooltip === 'light' ? 
             ...
             {Object.keys(data[0]).map((key, index)=> {
                 if(key != "name" && key != "stroke"){
                i++;
                 return <Line key={`data-${key}-${index}`} dataKey={key} stroke={colors[i]} stackId={props.stack ? 'stack-0' : `data-${key}`} dot={ props.dot ? { strokeWidth: 0, fill: colors[i], r: 2} : false} type={props.curve ? 'natural' : 'linear'} strokeWidth={props.thick ? 2 : 1}  />
                 } else {
                     return null
                 }
             })}
            ...
            {props.legend && <Legend align="right" verticalAlign="top" wrapperStyle={{ top: '-20px'}} /> }
            </LineChart>
        </ResponsiveContainer>
    </FlexiWrapper>
}

FlexiLine.defaultProps = {
    height: 200,  
